...
const ConversationItem: React.FC<Conversation> = ({
    ...
    >
        <div className="relative">
            <img 
                src={friend.avatar || `https://api.dicebear.com/9.x/identicon/svg?seed=${friend.username}`}
                alt={friend.username}
                className="size-10 rounded-full object-cover bg-violet-100 border border-violet-100"
            />
            <div
             className={`
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center">
                <p
                    className={
                        unreadMessages() ? 'text-sm text-gray-500 truncate min-h-[20px]' : 'text-sm text-violet-600 truncate min-h-[20px]'    
                    }
                >
                    {lastMessage?.content ?? ''}
                </p>
                {user && unreadCounts[user.id] > 0 && (
                    <div className="bg-violet-600 ml-2 text-xs text-white rounded-full size-5 flex items-center justify-center flex-shrink-0">
